...
(function () {
  ...
  const emfnDebug = (/** @type {unknown[]} */ ...args) => {
  ...
  const GeolocationFlow = {
    ...
    async handlePlaceSelection(event) {
      ...
      const addr = (place.addressComponents ?? []).reduce(
         ...
         * @param {object} root0
         * @param {string[]} root0.types
         * @param {string} root0.longText
         * @param {string} root0.shortText
  ...
  const RiskRenderer = {
     ...
     * @returns {Promise<NriCountyRow|null>} - matched county risk data
     */
    async fetchNriData(st, countyFIPS) {
      ...
      const countyMatch = matchedLine
        ? Object.fromEntries(
            heds.map((h, i) => [h.trim(), matchedLine.split(",")[i]?.trim() ?? ""])
          )
        : null;
  ...
  const SubmissionHashing = {
     ...
     * @property {string} category - name of the category in WP's database, from CSV's category column
     ...
     * @property {{[key: string]: HashedCategory[]}} registry - mapping of answerID to one or more category entries with ranks and catIDs
     * @property {Map<string, number>} categoryRanks - mapping of category name to its highest manual rank across all answers for ordering
     * @property {Map<string, number>} categoryIds - mapping of category name to its catID for encoding
     * @property {string[]} categoryOrder - first-seen order of categories as they appear in the CSV, used as a tiebreaker for equal ranks
     * @property {number[]} categoryIdOrder - parallel array to categoryOrder for efficient lookup of catIDs in the resolved order
     */
    ...
    /** @type {{[key: string]: HashedCategory[]}} */
