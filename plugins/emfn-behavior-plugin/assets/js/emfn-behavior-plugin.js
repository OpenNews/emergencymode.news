...
/** 
 * collect location data in a shape that would be useful for risk mapping, 
 * including the resolved FIPS code
 *
 * @typedef {Object.<string, string>} NriCountyRow
 ...
 * @property {NriCountyRow|null} nri - resolved via NRI data
 *
 * @typedef {Object} FccLookupResponse
 * @property {{FIPS?: string}=} County
 * @property {{code?: string}=} State
 *
 * @typedef {Object} PlaceAddressComponent
 * @property {string[]} types
 * @property {string} longText
 * @property {string} shortText
 *
 * @typedef {Object} PlaceLike
 * @property {Function} fetchFields
 * @property {Function=} location
 * @property {PlaceAddressComponent[]=} addressComponents
 *
 * @typedef {Object} GmpSelectEvent
 * @property {{toPlace: Function}} placePrediction
 */
...
/**
 * Gravity Forms gform/submission/submission_started filter payload.
 * @typedef {Object} GFormSubmissionStartedData
 * @property {HTMLFormElement} form - Form element being submitted.
 * @property {string} submissionType - Submission type provided by Gravity Forms.
 * @property {string} submissionMethod - Submission method provided by Gravity Forms.
 */
...
/** 
 * Intercept form submission via `gform/submission/submission_started` 
...
const controlFormSubmission = () => {
  const form = document.querySelector(gravityForm);
  if (!form || !window.gform || typeof window.gform.addFilter !== "function") {
    console.warn("Gravity Form not found; cannot control form submission.");
    return;
  }

  gform.addFilter("gform/submission/submission_started", 
    /** @param {GFormSubmissionStartedData} data */ (data) => 
  {
    ...
    const hashMarker = data.form.querySelector(".hashMarker");
    if (hashMarker) {
      hashMarker.value = hash;
      console.debug("actionPack hash set:", hash);
    } else {
      console.warn(".hashMarker input not found; actionPack will not appear in Confirmation URL.");
    }
 ...
 * @param {{lat: Function, lng: Function}} location - Google Places v2 location object
...
const initializeGeoFeatures = async () => {
  ...
  const buildOnAutocompleteEvent = () => {
    ...
    autocompleteEl.addEventListener("gmp-select", async (/** @type {GmpSelectEvent} */ { placePrediction }) => {
      /** @type {PlaceLike} */
...
(function () {
  ...
  if (window.gform !== undefined && window.gform.initializeOnLoaded) {
    window.gform.initializeOnLoaded(initializeGeoFeatures);
    window.gform.initializeOnLoaded(controlFormSubmission);
  } else {
